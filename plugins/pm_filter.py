...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('➕ Add Me To Your Groups ➕', url='http://t.me/md_filter_bot?startgroup=true')
            ],[
            InlineKeyboardButton('🔍 Search', switch_inline_query_current_chat=''),
            InlineKeyboardButton('🤖 Updates', url='https://t.me/malayalamdubbedmoviegroup')
            ],[
    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            for file in files:
                file_id = file.file_id
                btn.append(
                    [InlineKeyboardButton(text=f"{get_size(file.file_size)}", callback_data=f'files#{file_id}'), InlineKeyboardButton(text=f"{file.file_name}", callback_data=f'files_#{file_id}')]
                   )
        if not btn:
            return
